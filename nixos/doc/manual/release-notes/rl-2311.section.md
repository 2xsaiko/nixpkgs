...
- The `krb5` module has been rewritten and moved to `security.krb5`, removing `krb5.libdefaults`, `krb5.realms`, `krb5.domain_realm`, `krb5.capaths`, `krb5.appdefaults` and `krb5.plugins`, which have been replaced by `security.krb5.settings`. Additionally, `krb5.config` has been replaced by `security.krb5.extraConfig`.
