...
in {
  imports = [
    ...
    (mkRemovedOptionModule' "config" ''
      ...
      for structured configuration or `security.krb5.extraConfig' for plain-text
      configuration.
    '')
  ...
  options = {
    security.krb5 = {
      ...
      settings = mkOption {
        default = {};
        description = mdDoc ''
          Structured contents of the `krb5.conf` file. See `krb5.conf(5)`
      ...
      extraConfig = mkOption {
        ...
        '';
        description = mdDoc ''
          The verbatim contents of /etc/krb5.conf. `krb5.conf` may include any
          of the relations that are valid for `kdc.conf` (see `kdc.conf(5)`),
